...
df = pd.read_csv("../DATA/IMU-TRIM.TXT", sep=";", engine="python")
df = df.dropna()
...
fig, IMU = plt.subplots()
IMU.set_title("Données brutes de l'IMU")
IMU.set_xlabel("Temps (ms)")
IMU.set_ylabel("Accélération (m.s-2)")
IMU.plot(t, ax, label="ax")
IMU.plot(t, ay, label="ay")
IMU.plot(t, az, label="az")
IMU.plot(t, gx, label="gx")
IMU.plot(t, gy, label="gy")
IMU.plot(t, gz, label="gz")
IMU.grid()
IMU.legend()
...
# Initial attitude (sensor mounted sideways + rocket tilted 80°)
angleGlobal[0] = [-np.pi/2 - 80*np.pi/180, 0, 0]

# --- Filtering setup ---

# Bias correction from first 500 samples
gyro_bias = np.mean(wSelf[:500], axis=0)

# Low-pass filter strength
# alpha = 0.01  # small blending factor
alpha = 0.005

# Gravity norm for correction
g0 = 9.81
threshold = 0.5  # Acceptable deviation from 9.81
...
for i in range(1, N):
    ...
    # --- Tilt correction from accelerometer ---
    ...
    # --- Velocity decay when not accelerating significantly ---
    if abs(a_norm - g0) < threshold:
        vGlobal[i-1] *= decay
...
with open("./OUT/VOL-BLEND.txt", "w") as f:
    f.write(export)
